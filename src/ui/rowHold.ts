...
function slotTops(scene: readonly SceneBlock[]): (element: SceneBlock) => number {
  ...
  for (const element of scene) {
    ...
    if (stamp?.role === 'candidate' && element.type !== 'text') tops.set(slotKey(stamp.runId, stamp.nodeId, stamp.heading), top(element))
  }
  return element => {
    const hold = holdStamp(element)
    const pick = pickStamp(element)
    const key = hold && (hold.role === 'candidate' || hold.role === 'continue') ? slotKey(hold.runId, hold.nodeId, hold.heading)
...
export function rowHold(scene: readonly SceneBlock[], request: RowHoldRequest): RowHold | undefined {
  ...
  const candidate = live.find(element => ofSlot(element) && holdStamp(element)?.role === 'candidate' && element.type !== 'text')
