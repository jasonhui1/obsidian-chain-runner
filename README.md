...
**Three links, and none of them opens anything.** `⛓ Five Personas ▾` re-opens the chain picker and re-shapes the node around what you pick; `audience ▾ engineers` re-asks the chain's own options and rewrites the value where it stands; `▶ Run` runs the node. Every element of a node swallows its click, so a node that has been copied or half-deleted can never open a browser tab or make a note in the vault. Following a link on an Excalidraw canvas is **Ctrl/Cmd+click** (or the element's link icon), and all three lines answer to it.

**A plain click works too, on the two lines that only ask a question.** Clicking `⛓ Five Personas ▾` or `audience ▾ engineers` with no modifier opens their picker. Excalidraw reports a plain click only as a change of selection, so Chain Runner reads one element newly selected as a click — and then waits for the pointer to come back up before opening anything, because **dragging** the node begins with exactly the same event (ADR-0010). Selecting several, dragging, holding, selecting from the keyboard, or a redraw of the scene opens nothing. `▶ Run` is deliberately **not** on the single click: selecting a node is not asking to run it, and a run cannot be taken back where a picker can be dismissed. **Double-click it instead** — two clicks are a deliberate gesture in a way one selection is not. Ctrl/Cmd+click still starts a run too.
...
7g. **The two links.** Ctrl/Cmd+click `▶ Run`: expect the run to start, and **no** tab opened by the link itself. Ctrl/Cmd+click the dropdown line: expect the chain's own options, and the line rewritten in place on the pick. Reopen the drawing: expect the new value still there. Copy the node, set the copy's value, and expect the original's value unchanged.
...
| Re-cutting a resized node (step 7t) | **not run** — the re-cut itself is covered pure in `tests/chainNode.test.ts` (what it reveals, that it puts the type size back, that it leaves `▶ Run`'s words alone, and that a second re-cut at the same width writes nothing), and the release that triggers it in `tests/chainNodes.test.ts`. Only a vault can say whether the reveal is worth the drag: `GLYPH_WIDTH` is an estimate, not a measurement of Excalidraw's font (ADR-0009), so a re-cut line may sit short of the box's edge. Also unattested: that a group resize scales `fontSize` the way ADR-0011 assumes and that putting it back reads correctly, that `laidOut` really does keep an idle drawing from being saved on every pointer release, and that the height closing around the lines does not fight a reader who dragged a corner expecting both dimensions. |
| A plain click on a node (step 7s) | **partly run, 2026-09-04, `test_chain` vault on Excalidraw 2.26.4.** The anchored picker is attested: it opens beside the node with the drawing visible behind it, not centre-screen, and the CSS beats Obsidian's centring. Ctrl/Cmd+click still works. **A vault found the plain click did not open anything at all.** Excalidraw raises the selection through React's `onChange`, so the report arrives *after* `pointerup` — and the gate, which only queued callers while a press was in flight, saw no press and settled as "not a click" every time. A finished click is now held unclaimed for half a second, so both orders work; `tests/pointerClicks.test.ts` drives them. **Still not attested:** that the plain click now fires, that a double-click on `▶ Run` starts a run — the browser-counted route alone did not, because opening the text editor appears to clear the selection it asked about, so the run is now also taken from `editingTextElement`; and what that editor does to the line when it opens, that `pointerup` and `pointercancel` reach `document` in capture, whether the 4px/700ms thresholds and the 500ms window feel right under a hand, and that `autoResize: false` does not change how a node draws at rest. The rest of this row — the policy that decides which change of selection is a click is covered in `tests/selectionClick.test.ts`, whether the press behind it was a click or a drag in `tests/pointerClicks.test.ts`, where the picker opens in `tests/panelSpot.test.ts`, and the routing and its gate in `tests/chainNodes.test.ts`. Nothing below has touched a vault. `onSceneChangeHook`'s shape, its `appStateKeys` filter and its callback arguments were **read out of Excalidraw 2.26.4's `main.js`, not run** — so the first thing to check is that a plain click fires the hook at all, and that `previous?.callback` chaining does not break another plugin's hook. Also unattested: that the anchored picker's CSS actually beats Obsidian's own modal centring (`.prompt` is the element a suggest modal positions, and a theme may fight it), that a missed `pointerup` means a picker that never opens, and that `autoResize: false` on the node's lines does not change how a node draws at rest. |
