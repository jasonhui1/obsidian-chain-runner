...
| Expand (steps 7n-7p) | **not run** — where the cards land is covered pure in `tests/proposalFan.test.ts`, what a proposal is and what keeping or dropping one changes in `tests/proposal.test.ts`, and the order of an expansion in `tests/expand.test.ts`. No block has been expanded on a live drawing. Three things only a vault can attest: that `addArrow` binds a connector to the source block and the card (it is not in the spike's verified-call table), that `getViewSelectedElements` returns what the reader has selected, and that setting `isDeleted` on a copied element is how Excalidraw takes it off the scene. A build missing either call is refused with the version notice rather than failing quietly. |
