...
| The chain dropdown and the toolbar button (steps 7q, 7r) | **not run** — the re-shape is covered pure in `tests/chainNode.test.ts` and the two files in `tests/toolScript.test.ts`, but nothing has been pressed in a vault. Three things only a vault can attest: that a line drawn onto an existing node takes the node's group by having `groupIds` copied onto it, that an arrow bound into the node survives the box being resized, and that Excalidraw's script engine picks up a script this plugin wrote — including the `.svg` beside it, so the button is not a cog. |
