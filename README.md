...
Dragging a node's handles does not rewrite its type size: Excalidraw scales a text element by default, and the node's lines are drawn with that off, so a node keeps reading at the size the rest of the drawing was arranged around (ADR-0010). It does not reveal more of a long name — that line was trimmed when the node was built.
