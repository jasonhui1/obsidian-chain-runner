...
| Waiting hold column (#74) | **not run in a vault.** Pure layout, stamps, run flow and candidate clicks are covered in `tests/holdColumn.test.ts`, `tests/nodeRun.test.ts` and `tests/directFromDrawing.test.ts`. In a live drawing, run a chain that waits at a hold, check the unreached cards are gone and the full candidate text is readable, click a candidate, then close and reopen the drawing to check the bound text and stamps survived the save. Repeat with two waiting runs to check their frames stay separate. |
