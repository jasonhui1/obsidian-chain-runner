...
import { DEFAULT_SETTINGS, normaliseEngineUrl, normaliseOutputFolder, withDefaults } from '@/settings'
...
describe('withDefaults', () => {
  ...
  it('keeps a saved setting', () => {
    expect(withDefaults({ engineUrl: 'http://127.0.0.1:4000' })).toEqual({
      ...DEFAULT_SETTINGS,
...
describe('normaliseOutputFolder', () => {
  it('keeps a plain folder as it is', () => {
    expect(normaliseOutputFolder('notes/chain output')).toBe('notes/chain output')
  })

  it('drops the slashes at either end, so the run folder joins cleanly', () => {
    expect(normaliseOutputFolder('  /notes/runs/  ')).toBe('notes/runs')
  })

  it('falls back to the default rather than the vault root when the box is emptied', () => {
    expect(normaliseOutputFolder('  ')).toBe(DEFAULT_SETTINGS.outputFolder)
    expect(normaliseOutputFolder('/')).toBe(DEFAULT_SETTINGS.outputFolder)
  })
})
